...
package object nodescala {
  ...
  implicit class FutureCompanionOps[T](val f: Future.type) extends AnyVal {
    ...
    def all[T](fs: List[Future[T]]): Future[List[T]] = {
        ...
        def waitForNextFuture(acc: List[T], fsl: List[Future[T]]) : Unit = {
            fsl match {
                case Nil => p success acc.reverse
                case f :: fsp => f onComplete {
                    case Success(t) => waitForNextFuture(t :: acc, fsp)
                    case Failure(th) => p failure (th)
                }
            }
        }
        waitForNextFuture(List[T](), fs)
        p.future
    }
    */
    def all[T](fs: List[Future[T]]): Future[List[T]] = {
        (Future.always(List[T]()) /: fs)((acc, f) => for  {
            prevRes <- acc
            curRes <- f
        } yield prevRes :+ curRes)
    }
     ...
     */
    def delay(t: Duration): Future[Unit] = future { Await.ready(Future.never, t) }
    ...
    def run()(f: CancellationToken => Future[Unit]): Subscription = {
        ...
        val fut = f(subscription.cancellationToken)
        fut onSuccess { case r => }
        subscription
    }
  ...
  implicit class FutureOps[T](val f: Future[T]) extends AnyVal {
     ...
     */
    def now: T = try { Await.result(f, 0 nanos) } catch { case t: TimeoutException => throw new NoSuchElementException }
