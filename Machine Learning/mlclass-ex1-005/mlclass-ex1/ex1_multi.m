...
theta = zeros(3, 1);
alpha = 0.003
[theta, J_history7] = gradientDescentMulti(X, y, theta, alpha, num_iters);


theta = zeros(3, 1);
alpha = 0.001
[theta, J_history8] = gradientDescentMulti(X, y, theta, alpha, num_iters);
...
hold on;
plot(1:numel(J_history7), J_history7, '-m', 'LineWidth', 2);
hold on;
plot(1:numel(J_history8), J_history8, '-c', 'LineWidth', 2);
...
xin = [1650, 3]
for i=1:size(xin, 2)
	xin_norm(:, i) = xin(:, i) .- mu(1, i)
	xin_norm(:, i) = xin_norm(:, i) ./ sigma(1, i)
end;
xin_norm = [1, xin_norm]
price = xin_norm * theta
